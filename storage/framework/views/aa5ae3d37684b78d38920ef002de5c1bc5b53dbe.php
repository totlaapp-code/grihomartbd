...
<div class="px-4 pt-4 container-fluid">
                    ...
                    <div class="pt-1 pb-1 widget-rounded-circle card-box order" style="background: #0a0a7a;">
                        <div class="row">
                            <div class="col-12">
                                <div class="float-left">
                                    <h3 class="mt-1 mb-0 text-dark">
                                        <span id="allss"><?php echo e(App\Models\Order::where('admin_id',Auth::guard('admin')->user()->id)->get()->count()); ?></span>
                                    </h3>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body" style="text-align: center;">
                    <img src="<?php echo e(asset('public/tr.gif')); ?>" alt="" style="width: 224px;">
                </div>
                    ...
                    <div class="pt-4 pb-2 card-body">
                        <div class="row">
                            ...
                            <div class="col-8" style="text-align: right">
                                <a href="<?php echo e(url('admin/create/order')); ?>" class="btn btn-primary btn-sm"><span
                                            style="font-weight: bold;">+</span> Add New Order</a>
                                <?php if(request()->segment(count(request()->segments()))=='Ready to Ship'): ?>
                                    <?php if($admin->hasrole('superadmin') || $admin->hasrole('accounts') || $admin->hasrole('store')): ?>
                                    ...
                                    <div class="btn-group dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <?php $__currentLoopData = App\Models\Courier::where('status','Active')->get()->reverse(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $courier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <a class="dropdown-item assign-courier" data-id="<?php echo e($courier->id); ?>"
                                                    href="#"><?php echo e($courier->courierName); ?></a>
                                ...
                                <?php else: ?>
                                    <?php if($admin->hasrole('superadmin')): ?>
    ...
    <?php if(empty($status)): ?>
    <?php else: ?>
        <input type="text" id="orderstatus" value="<?php echo e($status); ?>" hidden>
    <?php endif; ?>
</div>
    ...
    $(document).ready(function() {
        ...
        if (user_role == '0') {
            var orderinfotbl = $('#orderinfo').DataTable({
                ajax: {
                    url: "<?php echo e(url('admin/admin_order/')); ?>" + '/' + orderstatus,
                },
                ordering: false,
                processing: true,
                serverSide: true,
        ...
        $(document).on('click', '.assign-user', function(e) {
            ...
            jQuery.ajax({
                type: "get",
                url: "<?php echo e(url('admin_order/assign_user')); ?>",
        ...
        $(document).on('click', '.btn-change-status', function(e) {
            ...
            $.ajax({
                type: "get",
                url: "<?php echo e(url('admin_order/statusUpdateByCheckbox')); ?>",
                data: {
                    'status': status,
                    'orders_id': ids,
                    '_token': token
        ...
        $(document).on('click', '.order-print-btn', function(e) {
            ...
            if (ids.length > 0) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo e(url('admin_order/store/Invoice')); ?>",
                    data: {
                        orders_id: ids
                    },
                    success: function(response) {
            ...
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            type: "GET",
                            url: "<?php echo e(url('admin_order/delete_selected_order')); ?>",
                            data: {
                                orders_id: ids,
                            },
                            success: function(response) {
        ...
        $("#userID").select2({
            ...
            allowClear: true,
            ajax: {
                url: '<?php echo e(url('admin_order/users')); ?>',
        ...
        $("#courierID").select2({
            ...
            allowClear: true,
            ajax: {
                url: '<?php echo e(url('admin_order/courier')); ?>',
                processResults: function(data) {
                    var data = $.parseJSON(data);
                    return {
                        results: data
        ...
        function countorder() {
            $.ajax({
                type: "get",
                url: "<?php echo e(url('admin_order/count')); ?>",
                contentType: "application/json",
                success: function(response) {
                    var data = JSON.parse(response);
        ...
        $(document).on('click', '.btn-status', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: "<?php echo e(url('order/admin_order/status')); ?>",
                data: {
                    'status': status,
                    'id': id,
                    '_token': token
        ...
        $(document).on('click', '.btn-editorder', function(e) {
            ...
            $.ajax({
                type: "get",
                url: "<?php echo e(url('admin_orders')); ?>/" + id + "/edit",
                success: function(response) {
                    ...
                    var orderCommentTable = $("#orderCommentTable").DataTable({
                        ajax: "<?php echo e(url('admin_order/getComment')); ?>?id=" + $(
                    ...
                    var oldOrderTable = $("#oldOrderTable").DataTable({
                        ajax: "<?php echo e(url('admin_order/previous_orders')); ?>?id=" + $(
                    ...
                    $(document).on("click", "#updateComment", function() {
                        ...
                        if (note.val() == '') {
                            ...
                            $.ajax({
                                type: "GET",
                                url: "<?php echo e(url('admin_order/updateComment')); ?>",
                                data: {
                                    'comment': note.val(),
                                    'id': id,
                                    '_token': token
        ...
        $(document).on("click", "#btn-update", function() {
            ...
            $.ajax({
                type: "PUT",
                url: "<?php echo e(url('admin_orders')); ?>/" + id,
                data: {
                    'data': data,
                    '_token': token
                },
        ...
        $(document).on('click', '.assign-courier', function(e) {
                ...
                jQuery.ajax({
                    type: "get",
                    url: "<?php echo e(url('admin_order/assign_courier')); ?>",
                    contentType: "application/json",
                    data: {
                        action: "assign",
                        ids: ids,
...
<?php if($admin->hasrole('user') || $admin->hasrole('manager')): ?>
    <style>
        .btn-delete {
            display: none;
        }
...
<?php echo $__env->make('backend.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/hazzamar/grihomartbd.com/resources/views/admin/content/order/order.blade.php ENDPATH**/ ?>